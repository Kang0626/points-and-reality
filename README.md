# ✨ Points & Reality 3DGS Pipeline Controller (v2.242)
...
* **Current Version**: `v2.242`
* **주요 변경 요약**:
  * **엔진 어트리뷰션 뱃지 전체화면 버튼 좌측 정렬 재배치 및 워터마크 행간-자간 2:1 황금비 최적화 (`v2.242`)**: 전체화면 버튼과 겹치지 않도록 플로팅 뱃지를 전체화면 버튼 바로 좌측(`bottom: 22px; right: 66px;`)에 완벽히 수직 중앙 정렬로 재배치하고, 워터마크의 행간을 자간 대비 2배(`row_gap = word_gap * 2`)로 조정하여 안정적이고 세련된 비주얼을 제공.
  * **엔진 어트리뷰션 뱃지 우측 하단 초슬림/고투명도 재배치 (`v2.241`)**: 시야를 가리지 않도록 뱃지를 화면 우측 하단으로 이동 배치하고, 35% 투명도(`opacity: 0.35`)와 초슬림 패딩으로 시각적 간섭을 최소화하면서 마우스 오버 시 부드러운 하이라이트(`opacity: 0.9`)를 지원하여 MIT 라이선스 준수와 시각적 몰입감을 동시에 만족.
